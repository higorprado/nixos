...
37. Split bundle features when hosts need different subsets. A feature that bundles fstrim + smartd requires a `mkForce` override on servers with no physical disks. Separate features (`maintenance.nix` for fstrim, `maintenance-smartd.nix` for smartd) let host inclusion be the condition, eliminating the only reason for `mkForce` in the codebase.
